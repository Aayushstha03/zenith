...
from zenith.health import health_report
...
def test_health_report_combines_dependencies(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("zenith.health.qdrant_health", lambda _: {"ready": True})
    monkeypatch.setattr("zenith.health.readiness", lambda _: {"ready": True})
...
def test_health_is_false_when_any_dependency_fails(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("zenith.health.qdrant_health", lambda _: {"ready": False})
    monkeypatch.setattr("zenith.health.readiness", lambda _: {"ready": True})
